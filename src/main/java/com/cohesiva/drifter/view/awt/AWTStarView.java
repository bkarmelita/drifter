...
public class AWTStarView extends AWTAbstractView<Star> {
	...
	private static final int[] RADIUS_BY_CLASS = new int[] {
		50, // for class O
		30, // for class B
		20, // for class A
		10, // for class F
	...
	private static final Color[] COLOR_BY_CLASS = new Color[] {
		Color.BLUE, 		// for class O
	...
	public void render(Star star) {
		...
		graphics.drawOval((int) display.x(), (int) display.y(), RADIUS_BY_CLASS[star.getStarClass().ordinal()], RADIUS_BY_CLASS[star.getStarClass().ordinal()]);
	}
